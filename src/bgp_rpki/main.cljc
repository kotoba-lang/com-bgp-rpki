(ns bgp-rpki.main
  "Kotodama WASM entrypoint for the bgp_rpki clean-room actor (L4) — Clojure port.
  ...
  facts (`bgp_rpki.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "bgp_rpki")
(def tier "L4")
...
(def entity-specs
  [{:entity "RouteOriginAuthorization" :plural "routeoriginauthorizations" :id-prefix "bgprpki_rou"
    :fields [:version :asn :prefix :prefixLength :maxLength :addressFamily] :required [:version :asn]
    :coerce {:version :int :asn :int :prefixLength :int :maxLength :int} :refs {}}
   {:entity "ValidationResult" :plural "validationresults" :id-prefix "bgprpki_val"
    :fields [:asn :prefix :prefixLength :validationState] :required [:asn :prefix]
    :coerce {:asn :int :prefixLength :int} :refs {}}
   {:entity "RpkiToRouterPdu" :plural "rpkitorouterpdus" :id-prefix "bgprpki_rpk"
    :fields [:protocolVersion :pduType :sessionId :serialNumber :length] :required [:protocolVersion :pduType]
    :coerce {:protocolVersion :int :pduType :int :sessionId :int :serialNumber :int :length :int} :refs {}}
   {:entity "PrefixPdu" :plural "prefixpdus" :id-prefix "bgprpki_pre"
    :fields [:protocolVersion :pduType :flags :prefixLength :maxLength :prefix :asn] :required [:protocolVersion :pduType]
    :coerce {:protocolVersion :int :pduType :int :flags :int :prefixLength :int :maxLength :int :asn :int} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"bgp_rpki.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "bgp_rpki-compat" :tier tier :entities entities} 200])
...
(defn start! [] :bgp_rpki-compat/ready)
